...
export const findNodeFunction = (type, id) =>
  n => {
    // console.info(`${n.type()} ${n.props().id}`)
    return n.type() === type && n.props().id === id
  }
...
export const clickMaterialUIButton = selector =>
  findNode(findNodeFunction('button', selector)).first().simulate('click')

export const clickMaterialUITab = selector =>
  clickMaterialUIButton(selector)

export const changeMaterialUITextEdit = (selector, value) =>
  findNode(findNodeFunction('input', selector)).first().simulate('change', { target: { value } })

export const simulateEnterMaterialUITextEdit = selector =>
  findNode(findNodeFunction('input', selector)).simulate('keypress', {key: 'Enter'})
